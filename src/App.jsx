// App.jsx – Notes restore + debug banner when ?debug=notes is in the URL.
import React, { useMemo, useState, useEffect } from "react";
import {
  Calendar, Users, Inbox as InboxIcon, Plus, Trash2, Search, FolderPlus,
...
function AppShell({ plannerEmail }) {
  ...
  // Debug banner: only show if ?debug=notes is present
  const debugNotes = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("debug") === "notes";

  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-white to-gray-50 p-6">
      {debugNotes && (
        <div className="mx-auto mb-3 max-w-6xl rounded-xl border border-yellow-300 bg-yellow-50 p-3 text-xs text-yellow-900">
          Build tag: <b>notes-visible</b>. If you can read this, you’re on the latest build. The Notes field should be under “2) Add tasks”.
        </div>
      )}
...
/* ---------------- Users Dashboard ---------------- */
// (unchanged from previously working version)
// ... (omitted here for brevity in this response)
// NOTE: Keep your existing UsersDashboard / InboxScreen implementations from the last working file.
// The only change that matters for Notes is inside TasksWizard + TasksEditorAdvanced below.
...
function TasksWizard({ plannerEmail, initialSelectedUserEmail = "" }) {
  ...
  useEffect(() => {
    function onPrefill(e){
      ...
      if (Array.isArray(data.tasks)) setTasks(data.tasks.map(t => ({ id: uid(), ...t })));
    }
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!initialSelectedUserEmail) {
        ...
        setSelectedUserEmail(connected?.email || any?.email || "");
      } else {
  ...
  return (
    <div className="mb-6">
      <div className="mb-2">
        ...
        <div className="text-xs text-gray-500">
          Click <b>Pick date</b> to open the calendar grid. That sets the task’s <b>date</b>. Choose Repeat, then <b>Add task(s)</b>.
        </div>
      </div>
    ...
    <div className="mb-3 flex items-center gap-2">
      ...
      {open && (
        <Modal onClose={()=>setOpen(false)} title="Choose a date">
          <CalendarGrid startDate={startDate} valueOffset={offset} onPickOffset={(o)=>{ setOffset(o); window.dispatchEvent(new CustomEvent("p2t:setBaseOffset",{ detail:{ offset: o }})); setOpen(false); }} />
        </Modal>
      )}
    </div>
  );
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        ...
        <button className="rounded-lg border px-2 py-1 text-xs" onClick={()=>{ setViewMonth(new Date(Date.UTC(start.getUTCFullYear(), start.getUTCMonth(), 1))); }}>
          Jump to plan start
        </button>
      </div>
      ...
      <div className="mt-2 text-xs text-gray-600">
        Window: {format(startUTC, "MMM d")} → {format(endUTC, "MMM d")} • Selected: {format(selectedUTC, "EEE MMM d")}
      </div>
    </div>
  );
}

/* ---------- Tasks editor (includes NOTES) ---------- */
function TasksEditorAdvanced({ startDate, onAdd }) {
  ...
  function generate() {
    ...
    // simple daily/weekly/monthly generation — same as previous working version
      ...
      {previewItems.length === 0 ? (
        ...
        <>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <button onClick={downloadICS} className="inline-flex items-center gap-2 rounded-xl border border-gray-300 bg-white px-4 py-2 text-sm hover:bg-gray-50">
