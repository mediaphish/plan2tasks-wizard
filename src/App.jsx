    ...
    <ol className="grid grid-cols-4 gap-2 mb-6">
      {STEPS.map((s, idx) => {
        ...
        return (
          <li key={s.key}>
            <button
              onClick={() => (done ? onJump(idx) : null)}
              className={cn(
                "w-full flex items-center gap-3 rounded-2xl p-3 border",
...
function Chip({ children, onRemove }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full border border-gray-300 bg-white px-2 py-1 text-xs">
      {children}
      {onRemove && (
        <button className="ml-1 text-gray-400 hover:text-gray-600" onClick={onRemove} aria-label="Remove">
          ×
        </button>
      )}
    </span>
    ...
    blocks.forEach((b) => {
      for (let d = 0; d < 7; d++) {
        ...
        if (b.days.includes(dow)) {
          out.push({ id: uid(), type: "block", title: b.label, dayOffset: d, time: b.time, durationMins: b.durationMins, notes: "Recurring block" });
        }
      }
    });
      ...
      <div className="mx-auto max-w-5xl">
        <header className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Guide users through creating a clean, shippable weekly plan.</p>
          </div>
            ...
            <AnimatePresence mode="wait">
              {step === 0 && (
                <motion.div key="s1" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                      <Field label="Plan title" required>
                        <input
                          ...
                          placeholder="e.g., Week of Aug 25"
                        />
                      </Field>
                  ...
                  <ActionBar canBack={false} canNext={canNext} onBack={back} onNext={next} />
                </motion.div>
              )}
              ...
              {step === 1 && (
                <motion.div key="s2" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                  ...
                  <ActionBar canBack canNext={canNext} onBack={back} onNext={next} />
                </motion.div>
              )}
              ...
              {step === 3 && (
                <motion.div key="s4" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                  <SectionCard title="Review & generate" description="Preview your week. Export to calendar or copy the Plan2Tasks block.">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-3">
                      ...
                      <button onClick={downloadICS} className="inline-flex items-center gap-2 rounded-xl border border-gray-300 bg-white px-4 py-2 text-sm font-semibold hover:bg-gray-50">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <SectionCard title="Plan basics" description="Name, dates, timezone">
              <div className="grid grid-cols-1 gap-4">
                <Field label="Plan title" required>
                  <input value={plan.title} onChange={(e) => setPlan({ ...plan, title: e.target.value })} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
                </Field>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Timezone" required>
                    <select value={plan.timezone} onChange={(e) => setPlan({ ...plan, timezone: e.target.value })} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500">
                      {TIMEZONES.map((tz) => (
                        <option key={tz} value={tz}>{tz}</option>
                      ))}
                    </select>
            ...
            <SectionCard title="Recurring blocks" description="Gym, meetings, etc.">
              <BlocksEditor blocks={blocks} setBlocks={setBlocks} />
            </SectionCard>

            <SectionCard title="Tasks" description="Add your tasks by day">
              <TasksEditor startDate={plan.startDate} tasks={tasks} setTasks={setTasks} />
            </SectionCard>
...
function BlocksEditor({ blocks, setBlocks }) {
  ...
  const add = () => {
    ...
    setBlocks([...blocks, { id: uid(), label: label.trim(), time, durationMins: Number(dur) || 60, days: [...days].sort() }]);
    setLabel("");
  };
    ...
    <div>
      <div className="mb-3 grid grid-cols-1 gap-3 md:grid-cols-4">
        <Field label="Label" required>
          <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g., Gym" className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
        </Field>
        <Field label="Time">
          <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
        </Field>
        <Field label="Duration (mins)">
          <input type="number" min={15} step={15} value={dur} onChange={(e) => setDur(e.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
        </Field>
      ...
      {blocks.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {blocks.map((b) => (
            <Chip key={b.id} onRemove={() => remove(b.id)}>
              {b.label} • {b.time} • {b.durationMins}m • {renderDays(b.days)}
            </Chip>
          ))}
        </div>
      )}
...
function TasksEditor({ startDate, tasks, setTasks }) {
  ...
  const add = () => {
    ...
    setTasks([...tasks, { id: uid(), title: title.trim(), dayOffset: Number(dayOffset) || 0, time: time || undefined, durationMins: Number(dur) || 60, notes }]);
    setTitle("");
    setNotes("");
  };
    ...
    <div>
      <div className="mb-3 grid grid-cols-1 gap-3 md:grid-cols-5">
        <Field label="Title" required>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Write proposal" className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
        </Field>
        <Field label="Day" hint="0 = start date">
          <select value={dayOffset} onChange={(e) => setDayOffset(e.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500">
            {[0, 1, 2, 3, 4, 5, 6].map((d) => (
              <option key={d} value={d}>{format(addDays(startDate, d), "EEE MM/dd")}</option>
            ))}
          </select>
        </Field>
        ...
        <Field label="Notes">
          <input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="optional" className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
        </Field>
      </div>
      <div className="flex items-center justify-between">
        <button onClick={add} className="inline-flex items-center gap-2 rounded-xl bg-cyan-600 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-700">
...
function addDays(startDateStr, d) {
      ...
      {[0, 1, 2, 3, 4, 5, 6].map((d) => (
        <div key={d} className="rounded-2xl border border-gray-200 bg-gray-50 p-3">
          <div className="mb-2 text-sm font-semibold text-gray-800">
            {format(addDays(startDate, d), "EEE MMM d")}
          </div>
...
export function renderPlanBlock({ plan, blocks, tasks }) {
  ...
  for (const b of blocks) {
    lines.push(`- ${b.label} | days=${b.days.join(",")} | time=${b.time} | dur=${b.durationMins}`);
  }
  lines.push("--- Tasks ---");
  for (const t of tasks) {
    lines.push(`- ${t.title} | day=${t.dayOffset} | time=${t.time || ""} | dur=${t.durationMins} | notes=${t.notes || ""}`);
  }
  lines.push("### END ###");
  return lines.join("\n");
}
