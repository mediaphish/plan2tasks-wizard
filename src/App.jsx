...
function InviteModal({ plannerEmail, userEmail: presetEmail, onClose }) {
  ...
  const [state, setState] = React.useState({
    loading: false, link: "", emailed: false, canSend: false, sending: false, error: "", fromLabel: ""
  });
  ...
  async function copy() {
    try { await navigator.clipboard.writeText(state.link); toast("Invite link copied"); }
    ...
    <Modal title="Invite user to connect Google Tasks" onClose={onClose}>
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={createLink} className="rounded-xl bg-cyan-600 px-3 py-2 text-xs font-semibold text-white hover:bg-cyan-700">
            Create Invite Link
          </button>
        ...
        <div className="border-t pt-2">
          {state.canSend ? (
            <div className="flex items-center justify-between">
              ...
              <button onClick={send} disabled={!state.link || state.sending}
    ...
    <Modal title="Settings" onClose={onClose}>
      <div className="flex items-center justify-between">
        ...
        <button onClick={toggle}
                className={clsx("rounded-full px-3 py-1.5 text-xs border",
                  autoArchive ? "bg-cyan-600 text-white border-cyan-600" : "hover:bg-gray-50")}>
          {autoArchive ? "On" : "Off"}
        </button>
      </div>
    </Modal>
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full items-center gap-2 sm:w-auto">
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search (email, categories, status)"
    ...
    <div className="flex flex-wrap gap-1.5">
      {days.map((d, idx) => {
        ...
        return (
          ...
          }} className={clsx("rounded-full px-2.5 py-1 text-xs border",
...
function PlanView({ plannerEmail, selectedUserEmail, onChangeUserEmail, onPushed }) {
  ...
  function addItem() {
    ...
    const timeParsed = newTime ? newTime : parseUserTimeTo24h(newTime);
    function withTime(d) {
      if (!d) return null;
      if (!timeParsed) return d;
      const [hh, mm] = timeParsed.split(":").map(n => parseInt(n, 10));
    ...
    if (recurring.type === "none") {
      ...
      while (added < n) {
        ...
        if (recurring.weeklyDays.includes(cur.getDay())) {
          batch.push({ title: newTitle.trim(), notes: newNotes.trim(), date: withTime(cur), time: timeParsed || "" }); added++;
        }
        cur = addDays(cur, 1);
      }
      ...
      >
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="block">
            ...
            <DateInput value={planDate} onChange={d => setPlanDate(d)}
        ...
        <div className="mt-2 grid gap-3 rounded-xl border border-gray-100 bg-gray-50 p-3 sm:grid-cols-2">
          <label className="block">
            ...
            <input value={listTitle} onChange={e => setListTitle(e.target.value)} placeholder="e.g., Onboarding – Week 1"
                   className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm" />
          </label>
          <div />
          ...
          <label className="block">
            ...
            <input value={newNotes} onChange={e => setNewNotes(e.target.value)} placeholder="Any extra details"
                   className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm" />
          </label>

          <div className="block">
            <div className="mb-1 text-xs font-medium">Task date</div>
            <DateInput value={newDate} onChange={d => setNewDate(d)}
          ...
          <div className="col-span-full rounded-xl border border-gray-200 bg-white p-3">
            ...
            <div className="flex flex-wrap gap-2">
              {["none", "daily", "weekly", "monthly"].map(t => (
                <button key={t} type="button" onClick={() => setRecurring(r => ({ ...r, type: t }))}
            ...
            {recurring.type === "monthly" ? (
              <div className="mt-2">
                ...
                <input type="number" min={1} max={31} value={recurring.monthlyDay || ""}
            ...
            {recurring.type !== "none" ? (
              <div className="mt-2 grid gap-2 sm:grid-cols-3">
                <label className="flex items-center gap-2 text-sm">
                  <input type="radio" name="rec-end" checked={recurring.end === "none"}
                         onChange={() => setRecurring(r => ({ ...r, end: "none", until: null }))} />
                  <span>Indefinite (limited preview)</span>
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <input type="radio" name="rec-end" checked={recurring.end === "count"}
                         onChange={() => setRecurring(r => ({ ...r, end: "count", until: null }))} />
                  <span>Generate <input type="number" min={1} value={recurring.count}
                                        onChange={e => setRecurring(r => ({ ...r, count: Number(e.target.value || 1) }))}
                                        className="mx-1 w-16 rounded-lg border px-2 py-1 text-sm" /> items</span>
                </label>
                <div className="flex items-center gap-2">
                  <label className="flex items-center gap-2 text-sm">
                    <input type="radio" name="rec-end" checked={recurring.end === "until"}
          ...
          <div className="col-span-full flex items-center gap-2">
            <button onClick={addItem} className="rounded-xl bg-gray-900 px-3 py-2 text-sm font-semibold text-white hover:bg-black">
              Add task
            </button>
          </div>
        </div>
...
function AppShell() {
  ...
  // Load current session once, and listen for changes. NO redirect loop.
  React.useEffect(() => {
    let mounted = true;
    (async () => {
      try {
  ...
  async function logout() {
    ...
    finally { /* stay on page; gate below will show */ }
  }

  // Gate: not signed in → simple screen (no redirect)
  if (ready && !session) {
    return (
  ...
  return (
    <div className="mx-auto max-w-6xl p-3 sm:p-6">
      ...
      {tab === "users" ? (
        <UsersView plannerEmail={plannerEmail} onManage={(email) => { /* go to plan with selection */ setTab("plan"); }} />
      ) : (
        <PlanView
          plannerEmail={plannerEmail}
          selectedUserEmail={"" /* user is chosen inside Plan view dropdown */}
          onChangeUserEmail={() => {}}
          onPushed={() => {}}
        />
      )}
